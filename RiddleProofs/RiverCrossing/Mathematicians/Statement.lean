...
def notebook_safe (s : MathematicianState) : Bool :=
  decide (∀ (owner : Fin num_mathematicians),
    let notebook_bank := s.possession_bank[owner]!
    let mathematician_bank := s.owner_bank[owner]!
    ...
    ((notebook_bank = mathematician_bank) ∨
     (∀ (other : Fin num_mathematicians), other ≠ owner → s.owner_bank[other]! ≠ notebook_bank)))
...
theorem final_safe: notebook_safe final_state  := by
  unfold notebook_safe
  simp
